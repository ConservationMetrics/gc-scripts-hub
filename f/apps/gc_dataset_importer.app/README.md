...
- Displays warning if table exists, indicating data will be appended
- User can proceed with append or change the dataset name
...
flowchart TD
 ...
 %% Upload + validation
    ...
    CheckDataset --> Exists["⚠️ Will append to existing dataset"] & NotExists["✅ New dataset"]
...
- Connects to PostgreSQL warehouse to check if table exists
- Returns table existence status along with user-friendly and SQL-safe dataset names
- Displays warning message if table exists, allowing user to proceed with append
...
1. Enter table name: `community_surveys` (already exists)
2. System warns: "This dataset already exists. New data will be appended."
3. Upload: `additional_surveys.csv`
4. Select source: _none selected_
5. Result: New rows appended to existing `community_surveys` table

> [!NOTE]
> Append logic is handled by the `StructuredDBWriter._safe_insert` method in the `common_logic.db_operations` module, which is also used by the connector scripts.
...
- [ ] When appending to existing tables, validate schema compatibility and provide clear error messages if columns don't match
