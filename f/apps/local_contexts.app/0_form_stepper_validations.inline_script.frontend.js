...
if (currentStepIndex === 1) {
  ...
  const alreadyApplied = [
    ...(Array.isArray(state.tkLabelsAlreadyApplied)
      ? state.tkLabelsAlreadyApplied
      : []),
    ...(Array.isArray(state.bcLabelsAlreadyApplied)
      ? state.bcLabelsAlreadyApplied
      : []),
  ];
  ...
  const hasRemovedLabel = alreadyApplied.some(
    (label) => !selectedSet.has(label),
  );

  if (!hasNewLabel && !hasRemovedLabel) {
    throw new Error(
      "Please make at least one Label change (add or remove) before continuing.",
    );
  }
}
...
if (currentStepIndex === 2 && state.finalizeSuccess) {
  throw new Error(
    "Please refresh the page to apply labels to another dataset.",
  );
}
